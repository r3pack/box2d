...
void b2MouseJoint_SetSpringHertz( b2JointId jointId, b2Float hertz )
{
...
b2Float b2MouseJoint_GetSpringHertz( b2JointId jointId )
{
	b2JointSim* base = b2GetJointSimCheckType( jointId, b2_mouseJoint );
	return base->mouseJoint.hertz;
}

void b2MouseJoint_SetSpringDampingRatio( b2JointId jointId, b2Float dampingRatio )
{
...
b2Float b2MouseJoint_GetSpringDampingRatio( b2JointId jointId )
{
	b2JointSim* base = b2GetJointSimCheckType( jointId, b2_mouseJoint );
	return base->mouseJoint.dampingRatio;
}

void b2MouseJoint_SetMaxForce( b2JointId jointId, b2Float maxForce )
{
...
b2Float b2MouseJoint_GetMaxForce( b2JointId jointId )
{
	b2JointSim* base = b2GetJointSimCheckType( jointId, b2_mouseJoint );
...
b2Float b2GetMouseJointTorque( b2World* world, b2JointSim* base )
{
...
void b2PrepareMouseJoint( b2JointSim* base, b2StepContext* context )
{
	...
	b2Float angularHertz = 0.5f;
	b2Float angularDampingRatio = 0.1f;
...
void b2SolveMouseJoint( b2JointSim* base, b2StepContext* context )
{
	...
	{
		b2Float massScale = joint->angularSoftness.massScale;
		b2Float impulseScale = joint->angularSoftness.impulseScale;

		b2Float impulse = iB > 0.0f ? -wB / iB : 0.0f;
	...
	b2Float maxImpulse = joint->maxForce * context->h;

	{
		...
		b2Float massScale = joint->linearSoftness.massScale;
		b2Float impulseScale = joint->linearSoftness.impulseScale;
		...
		b2Float mag = b2Length( joint->linearImpulse );
		if ( mag > maxImpulse )
		{
