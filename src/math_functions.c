...
b2Float b2Atan2( b2Float y, b2Float x )
{
	...
	b2Float ax = b2AbsFloat( x );
	b2Float ay = b2AbsFloat( y );
	b2Float mx = b2MaxFloat( ay, ax );
	b2Float mn = b2MinFloat( ay, ax );
	b2Float a = mn / mx;
	...
	b2Float c = s * a;
	b2Float q = s * s;
	b2Float r = 0.024840285f * q + 0.18681418f;
	b2Float t = -0.094097948f * q - 0.33213072f;
	r = r * s + t;
	r = r * c + a;
...
b2CosSin b2ComputeCosSin( b2Float radians )
{
	b2Float x = b2UnwindLargeAngle( radians );
	b2Float pi2 = B2_PI * B2_PI;
