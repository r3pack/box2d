...
B2_INLINE b2Float b2MinFloat( b2Float a, b2Float b )
{
	return a < b ? a : b;
}

/// @return the maximum of two floats
B2_INLINE b2Float b2MaxFloat( b2Float a, b2Float b )
{
	return a > b ? a : b;
}

/// @return the absolute value of a b2Float
B2_INLINE b2Float b2AbsFloat( b2Float a )
{
	return a < 0 ? -a : a;
}

/// @return a b2Float clamped between a lower and upper bound
B2_INLINE b2Float b2ClampFloat( b2Float a, b2Float lower, b2Float upper )
{
	return a < lower ? lower : ( a > upper ? upper : a );
}
...
B2_API b2Float b2Atan2( b2Float y, b2Float x );

/// Compute the cosine and sine of an angle in radians. Implemented
/// for cross-platform determinism.
B2_API b2CosSin b2ComputeCosSin( b2Float radians );

/// Vector dot product
B2_INLINE b2Float b2Dot( b2Vec2 a, b2Vec2 b )
{
	return a.x * b.x + a.y * b.y;
}

/// Vector cross product. In 2D this yields a scalar.
B2_INLINE b2Float b2Cross( b2Vec2 a, b2Vec2 b )
{
	return a.x * b.y - a.y * b.x;
}

/// Perform the cross product on a vector and a scalar. In 2D this produces a vector.
B2_INLINE b2Vec2 b2CrossVS( b2Vec2 v, b2Float s )
{
	return B2_LITERAL( b2Vec2 ){ s * v.y, -s * v.x };
}

/// Perform the cross product on a scalar and a vector. In 2D this produces a vector.
B2_INLINE b2Vec2 b2CrossSV( b2Float s, b2Vec2 v )
{
	return B2_LITERAL( b2Vec2 ){ -s * v.y, s * v.x };
}
...
B2_INLINE b2Vec2 b2Lerp( b2Vec2 a, b2Vec2 b, b2Float t )
{
...
B2_INLINE b2Vec2 b2MulSV( b2Float s, b2Vec2 v )
{
	return B2_LITERAL( b2Vec2 ){ s * v.x, s * v.y };
}

/// a + s * b
B2_INLINE b2Vec2 b2MulAdd( b2Vec2 a, b2Float s, b2Vec2 b )
{
	return B2_LITERAL( b2Vec2 ){ a.x + s * b.x, a.y + s * b.y };
}

/// a - s * b
B2_INLINE b2Vec2 b2MulSub( b2Vec2 a, b2Float s, b2Vec2 b )
{
	return B2_LITERAL( b2Vec2 ){ a.x - s * b.x, a.y - s * b.y };
}
...
B2_INLINE b2Float b2Length( b2Vec2 v )
{
	return b2Sqrt( v.x * v.x + v.y * v.y );
}

/// Get the distance between two points
B2_INLINE b2Float b2Distance( b2Vec2 a, b2Vec2 b )
{
	b2Float dx = b.x - a.x;
	b2Float dy = b.y - a.y;
...
{
	b2Float length = b2Sqrt( v.x * v.x + v.y * v.y );
	if ( length < FLT_EPSILON )
	{
		return b2Vec2_zero;
	}

	b2Float invLength = 1.0f / length;
...
/// outputs the length.
B2_INLINE b2Vec2 b2GetLengthAndNormalize( b2Float* length, b2Vec2 v )
{
	*length = b2Length( v );
	if ( *length < FLT_EPSILON )
	{
...
{
	b2Float mag = b2Sqrt( q.s * q.s + q.c * q.c );
	b2Float invMag = mag > 0.0 ? 1.0f / mag : 0.0f;
...
B2_INLINE b2Rot b2IntegrateRotation( b2Rot q1, b2Float deltaAngle )
{
	// dc/dt = -omega * sin(t)
	// ds/dt = omega * cos(t)
	// c2 = c1 - omega * h * s1
	...
	b2Float mag = b2Sqrt( q2.s * q2.s + q2.c * q2.c );
	b2Float invMag = mag > 0.0 ? 1.0f / mag : 0.0f;
...
B2_INLINE b2Float b2LengthSquared( b2Vec2 v )
{
	return v.x * v.x + v.y * v.y;
}

/// Get the distance squared between points
B2_INLINE b2Float b2DistanceSquared( b2Vec2 a, b2Vec2 b )
{
...
B2_INLINE b2Rot b2MakeRot( b2Float radians )
{
...
B2_INLINE b2Rot b2NLerp( b2Rot q1, b2Rot q2, b2Float t )
{
	b2Float omt = 1.0f - t;
	b2Rot q = {
		omt * q1.c + t * q2.c,
...
B2_INLINE b2Float b2ComputeAngularVelocity( b2Rot q1, b2Rot q2, b2Float inv_h )
{
	// ds/dt = omega * cos(t)
	// dc/dt = -omega * sin(t)
	// s2 = s1 + omega * h * c1
	...
	b2Float omega = inv_h * ( q2.s * q1.c - q2.c * q1.s );
	return omega;
}

/// Get the angle in radians in the range [-pi, pi]
B2_INLINE b2Float b2Rot_GetAngle( b2Rot q )
{
...
B2_INLINE b2Float b2RelativeAngle( b2Rot b, b2Rot a )
{
	// sin(b - a) = bs * ac - bc * as
	// cos(b - a) = bc * ac + bs * as
	b2Float s = b.s * a.c - b.c * a.s;
	b2Float c = b.c * a.c + b.s * a.s;
	return b2Atan2( s, c );
}

/// Convert an angle in the range [-2*pi, 2*pi] into the range [-pi, pi]
B2_INLINE b2Float b2UnwindAngle( b2Float radians )
{
	if ( radians < -B2_PI )
	{
...
B2_INLINE b2Float b2UnwindLargeAngle( b2Float radians )
{
	while ( radians > B2_PI )
	{
		radians -= 2.0f * B2_PI;
...
{
	b2Float x = ( t.q.c * p.x - t.q.s * p.y ) + t.p.x;
	b2Float y = ( t.q.s * p.x + t.q.c * p.y ) + t.p.y;
...
{
	b2Float vx = p.x - t.p.x;
	b2Float vy = p.y - t.p.y;
...
{
	b2Float a = A.cx.x, b = A.cy.x, c = A.cx.y, d = A.cy.y;
	b2Float det = a * d - b * c;
	if ( det != 0.0f )
	{
		det = 1.0f / det;
...
{
	b2Float a11 = A.cx.x, a12 = A.cy.x, a21 = A.cx.y, a22 = A.cy.y;
	b2Float det = a11 * a22 - a12 * a21;
	if ( det != 0.0f )
	{
		det = 1.0f / det;
	}
...
B2_API bool b2IsValidFloat( b2Float a );
...
B2_API void b2SetLengthUnitsPerMeter( b2Float lengthUnits );

/// Get the current length units per meter.
B2_API b2Float b2GetLengthUnitsPerMeter( void );
...
inline void operator*=( b2Vec2& a, b2Float b )
{
	a.x *= b;
	a.y *= b;
}
...
inline b2Vec2 operator*( b2Float a, b2Vec2 b )
{
	return { a * b.x, a * b.y };
}

/// Binary scalar and vector multiplication
inline b2Vec2 operator*( b2Vec2 a, b2Float b )
{
	return { a.x * b, a.y * b };
}
